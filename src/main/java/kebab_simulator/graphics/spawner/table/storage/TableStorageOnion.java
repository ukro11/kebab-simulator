...
public class TableStorageOnion extends TableStorageSpawner {

    public TableStorageOnion(ObjectIdResolver id, Collider collider) {
        super(TableStorageType.ONION, id, collider);
    }

    @Override
    public void onInteractTable(EntityPlayer player, KeyEvent event) {
        if (event.getKeyCode() == KeyManagerModel.KEY_TAKE_ITEM.getKey()) {
            player.getInventory().pickItem(Wrapper.getEntityManager().spawnOnion());
