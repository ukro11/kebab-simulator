...
public class TableStorageTomato extends TableStorageSpawner {

    public TableStorageTomato(ObjectIdResolver id, Collider collider) {
        super(TableStorageType.TOMATO, id, collider);
    }

    @Override
    public void onInteractTable(EntityPlayer player, KeyEvent event) {
        if (event.getKeyCode() == KeyManagerModel.KEY_TAKE_ITEM.getKey()) {
            player.getInventory().pickItem(Wrapper.getEntityManager().spawnTomato());
